...
ggplot(df_top_countries, aes(mean_lap, fill=factor(race_name))) +
  geom_density(alpha=0.5) +
  labs(title = "Price distribution for flats in different conditions")

ggplot(df_top_countries, aes(race_name, mean_lap)) + 
       geom_line() +
       geom_point() + 
       #geom_smooth(method = 'lm', colour='cyan4') +
       labs(title='Scatter Plot', x='Race Track', y='Mean of Laps') +
       theme_classic()

df_box <- df[,
              .(race_name, laps) ]
df_box <- df_box %>% top_n(20)
ggplot(df_box, aes(x=factor(race_name), y=laps)) +
  geom_boxplot(color= "#006D77", fill= "#66B7B0", alpha=0.7) +
  labs(x="District") +
  theme_bw()

```
The number of laps in a race is obtained by dividing 305 by the length of a lap, which differs from track to track. The duration of the race cannot be more than 2 hours. If the allocated time of 2 hours is exceeded, the race is considered to be finished at the end of the ongoing lap. The figure above shows the average laps for the top 10 tracks. 
...
title: "Final Project for Data Visualization"
author: "Fatima Arshad"
date: "2/23/2022"
